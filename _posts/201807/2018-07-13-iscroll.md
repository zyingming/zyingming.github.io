...
## 写在前头
`iscroll`针对不同的使用场景，分了几个版本，在这个demo中使用的是`iscroll-infinite.js`。`iscroll`不是如之前用到的插件那样可以直接调用api，而是利用里面的变量和函数完成自定义事件。对`html`和`css`都有固定的要求。在测试的时候，官网给出的例子中也有一些问题，坑很多。在这个demo中使用[mock](https://github.com/nuysoft/Mock/wiki/Getting-Started)模拟数据请求，[demo源码地址]()。
...
### 6. 如果在无内容时有提示文字，在上拉刷新滚动后，文字不能回到原来的位置
